...
---
#46、ereg_replace和eregi_replace有什么区别？ P
...
---
#104、有一个一维数组，里面存储整形数据，请写一个函数，将他们按从大到校的顺序排列。要求执行效率高。并说明如何改善执行效率。（该函数必须自己实现，不能使用PHP函数） P
...
---
#109、字符串转数组,数组转字符串,字符串截取,字符串替换,字符串查找的函数分别是什么? P
...
---
#154、在数据库test中的一个表student，字段是name，class，score。分别代表姓名、所在班级，分数。 P
...
---
#172、不断在文件hello.txt头部写入一行"Hello World"字符串，要求代码完整。 P 
...
---
#179、什么是数据库索引，主键索引，唯一索引的区别，索引的缺点是什么？ M
...
---
#225、一个字节占多少bit ? 一个IPv4地址占几个字节? 一个IPv6地址呢?undone P
...
在<VirtualHost></VirtualHost>中间重启APACHE即可.
...
---
#288、HTTP协议中几个状态码的含义:503 500 401 403 404 200 301 302。。。 P
...
---
#289、PHP/Mysql中几个版本的进化史，比如mysql4.0到4.1，PHP 4.x到5.1的重大改进等等。undone P
...
---
#296、写代码来解决多进程/线程同时读写一个文件的问题。 P

php多进程编程，利用pcntl和posix
...
---
#298、Mysql 的存储引擎,myisam和innodb的区别。 M
...
---
#299、介绍xdebug,apc,eAccelerator,Xcache,Zend opt的使用经验。undone P

xdebug是php调试工具
apc是缓存工具

---
#300、MySQL数据库作发布系统的存储，一天五万条以上的增量，预计运维三年,怎么优化？ M
...
---
#302、 请简单阐述您最得意的开发之作。undone P
浪首、汽首IP定向功能
CMS系统，扩展、优化、函数，新项目规划
QA项目，redis坑，1）备份时会新开一块全量数据的内存空间，导致内存不足，项目瘫痪。2）在取两个标签并集时会生成一个新的set存储数据，而且没有过期时间，浪费大量内存。
图库，压图机制优化，利用mysql锁机制做队列，4台压图服务器各开10个php进程循环读mysql中的队列，找到未压的图片就压图并将该条数据的status更新为在处理，压完后更新为处理完成。

---
#303、对于大流量的网站,您采用什么样的方法来解决各页面访问量统计问题？ P

统一布码监测

---
#304、请介绍Session的原理,大型网站中Session方面应注意什么? undone P
...
---
#305、测试php性能和mysql数据库性能的工具,和找出瓶颈的方法？undone P

php性能测试
- XHProf，一个分层PHP性能分析工具。它报告函数级别的请求次数和各种指标，包括 阻塞时间，CPU时间和内存使用情况。
- WebBench，最多可以模拟3万个并发连接去测试网站的负载能力。

mysql性能测试
- Mysqlslap，Mysqlslap是从5.1.4版开始的一个MySQL官方提供的压力测试工具。通过模拟多个并发客户端访问MySQL来执行压力测试，同时详细的提供了“高负荷攻击MySQL”的数据性能报告。并且能很好的对比多个存储引擎在相同环境下的并发压力性能差别。


---
#306、 介绍一下常见的SSO(单点登陆)方案(比如dedecms整合discuz的passport)的原理。undone P


---
#307、您写过的PHP框架的特点，主要解决什么问题，与其他框架的不同点。undone P


---
#308、大型的论坛/新闻文章系统/SNS网站在性能优化上有什么区别? undone P


---
#309、相册类应用:要求在浏览器中能同时选中并上传多个文件，图片要求能剪裁，压缩包在服务器端解压。能上传单个达50M的文件。上传过程中有进度条显示。每个图片能生成四种大小缩略图，视频文件要转成flv供flash播放。叙述要涉及的各类开源软件和简单用途。？ P

同时选中和上传多文件，用form表单可以完成，type=file。
文件上传限制，php.ini中max_execution_time（默认30秒）、post_max_size、upload_max_filesize
上传进度条，php5.2新增功能，用 $_POST 数组和 APC_UPLOAD_PROGRESS 创建实时进度条。开启APC，设置apc.max_file_size，利用apc_cache函数获取缓存中的progress变量，返回值是一个数组，包括文件总大小，目前为止收到的文件数、上传速度等，将计算结果返回给前端。
上传插件，jquery-file-upload
文件解压，用php zip函数
图像剪裁、处理，前端jcrop，后端处理image、Imagine开源库，利用gd、imagick或gmagick
视频处理，ffmpeg

---
#310、一群猴子排成一圈，按1，2，…，n依次编号。然后从第1只开始数，数到第m只,把它踢出圈，从它后面再开始数，再数到第m只，在把它踢出去…，如此不停的进行下去，直到最后只剩下一只猴子为止，那只猴子就叫做大王。要求编程模拟此过程，输入m、n, 输出最后那个大王的编号。用程序模拟该过程。 P
...
---
#311、linux下查看当前系统负载信息的一些方法。L 

w，Uptime	查看系统运行时间、用户、负载
vmstat	使用vmstat命令可以得到关于进程、内存、内存分页、堵塞IO、traps及CPU活动的信息。
top	查看系统整体状态
...
---
#313、ssh安全增强方法，密码方式和rsa key方式的配置。L
...
密码方式
dsa和rsa，都是非对称加密算法，前者只能用于数字签名，不能用于加密，后者都行。

rsa配置
1. 客户端创建密钥对， ssh-keygen -t rsa
2. 将公钥复制到服务器端， scp id_rsa.pub root@192.168.36.194:~/.ssh/authorized_keys
3. 访问服务器，ssh 192.168.36.194

---
#314、 rpm/apt/yum/ports 装包，查询，删除的基本命令。 L
...
---
#315、Makefile的基本格式，gcc编译，连接的命令,-O0 和-O3区别。undone L


---
#316、gdb,strace,valgrind的基本使用. undone L
...
---
#319、javascript中this对象的作用域。 J

This是执行上下文环境的一个属性，而不是某个变量对象的属性。

---
#320、IE和firefox事件冒泡的不同。 undone J


---
#321、什么是怪异模式,标准模式，近标准模式。undone J
...
---
#324、firefox,IE下的前端js/css调试工具。 J

firebug、WebDeveloper

---
#325、面试官看过你的简历，会问一些你做的项目的用户量、pv、吞吐量、相关难点和解决方法等。 undone P


---
#326、数据库设计经验,为什么进行分表? 分库? undone M

一般多少数据量开始分表? 分库? 分库分表的目的? 什么是数据库垂直拆分? 水平拆分? 分区等等？可以举例说明

---
#327、web开发方面会遇到哪些缓存? 分别如何优化?  undone P

memcahed和redis

---
#328、给你256M的内存,对10G的文件进行排序(文件每行1个数字),如何实现？ 对10G的文件进行查找如何实现？ 统计10G文件每个关键字出现的次数如何实现？ undone P

300万个字符串最多（假设没有重复，都是最大长度）占用内存3M*1K/4=0.75G。所以可以将所有字符串都存放在内存中进行处理。
可以使用key为字符串（事实上是字符串的hash值），值为字符串出现次数的hash来统计每个每个字符串出现的次数。
并用一个长度为10的数组/链表来存储目前出现次数最多的10个字符串。
这样空间和时间的复杂度都是O(n)。

---
#329、假如你现在是12306火车订票的设计师,你该如何设计满足全国人民订票? undone P

---
#330、假如有1亿用户的访问量,你的服务器架构是怎样的? 用户信息的存储方案如何设计?  undone P

---
#331、如果你是技术组长,所带团队任务进度无法完成你该如何解决? undone P 

---
#332、如果在进度排满的前提下插入任务,你该如何保证总进度不延期? undone P

---
#332、如果有的工程师今天预定任务没有完成,你该如何解决? undone P

---
#332、从你的经验方面谈一下如何构建高性能web站点? 需要哪些环节? 步骤? 每个步骤需要注意什么如何优化等?  undone P 

---
#333、 如何处理多服务器共享session?  P 
1. 把原来存储在服务器磁盘上的session数据存储到客户端的cookie中去。
2. 用一种算法分配用户访问指定服务器，实现一种基于session的负载均衡。但如果某服务器挂了，分配到这个服务器的用户就无法使用session了。
3. 做一个中间层，专门来存储所有访问涉及到的session。也就是所有的session都存储在这里。服务器端统一从这里读取session数据，利用memcache或redis。

---
#334、 一个10G的表,你用php程序统计某个字段出现的次数,思路是?  undone P

---
#335、会告诉你一个nginx日志例子,用你认为最佳的编程语言统计一下http响应时间超过1秒的前10个url?  undone P

---
#336、给你一个mysql配置文件,用你认为最佳的编程语言解析该文件? undone P

---
#337、给你一个url,在nginx配置一下rewrite指定到某个具体路径? undone A

---
#338、一个php文件的解释过程是? 一般加速php有哪些?  提高php整体性能会用到哪些技术? undone P
...
---
#339、chrome号称为多线程的,所以多线程和多进程的区别为? P 
...
---
#340、 php在2011年底出现hash碰撞,hash碰撞原理为? 如何进行修复?
...
---
#341、web不安全因素有哪些? 分别如何防范? undone P
...
---
#342、假如两个单链表相交,写一个最优算法计算交点位置,说思路也可以?  undone P

---
#343、nginx负载均衡有哪些? 如果其中一台服务器挂掉,报警机制如何实现? undone A

---
#344、不优化前提下,apache一般最大连接数为? nginx一般最大连接数为? mysql 每秒insert ? select ? update ? delete? undone A

apache默认最大连接数256，nginx默认是1024

---
#345、mysql 数据类型有哪些 ? 分别占用多少存储空间 ? undone M

---
#346、nginx设置缓存js、css、图片等信息,缓存的实现原理是? undone A


---
#347、如何提高缓存命中率? 如何对缓存进行颗粒化?  undone P 
...
---
#349、linux监视一个日志文件a.log 的最新内容? L

tail –f a.log查看文件最后10行，动态显示。

---
#350、查看当前服务器的HTTP连接有哪些？ L
...
---
#351、说说Linux下的find命令和grep命令的区别？ L

Find查找文件或目录
Grep 在文件内容中/输出结果中查找字符串

---
#352、如何实现每天0点钟重新启动服务器?  L
...
---
#353、请写出5个常用的unix操作命令? L

Ls cp touch mkdir rm cat kill ps

---
#354、rm -rf * 是什么意思，其中 f 有什么作用? L

–r表示递归（删除目录），f强制（可以不提示删除），* 通配符
删除当前目录下所有文件

---
#355、Linux压缩文件和解压文件的命令? L

gzip  tar  bzip  zip
gunzip  tar  bunzip  unzip

---
#356、Linux的目录进行遍历，编写shell脚本? L

ls –R
tree 以树状结构显示目录

---
#357、apache默认使用进程管理还是线程管理？如何判断并设置最大连接数？ A
...
---
#358、当前目录下有一个文件为 showme.sh , 如何修改文件，将其指定为使用 /bin/bash 运行，如何修改其权限为所有用户可读写，所有用户可执行? L
...
---
#359、当前目录下有一个文件为 showme.sh,如何修改其所有人为root? L

Chown root showme.sh

---
#360、在linux 请写出一种自动定时备份某一个文件的方法和具体实施流程，要求每天凌晨3 点的时候备份一次。 undone L
